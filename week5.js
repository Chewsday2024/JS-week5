    ...
    ticketHtml += `
      <li class="ticketCard">
        <div class="ticketCard-img">
          <a href="#">
            <img src="${value.imgAddress}" alt="">
          </a>
          <div class="ticketCard-region">${value.viewLocation}</div>
          <div class="ticketCard-rank">${value.ticketStars}</div>
        </div>
        <div class="ticketCard-content">
          <div>
            <h3>
              <a href="#" class="ticketCard-name">${value.ticketName}</a>
            </h3>
          ...
          <div class="ticketCard-info">
            <div class="ticketCard-num">
              <p>
                ...
                剩下最後 <span id="ticketCard-num"> ${value.ticketQuantity} </span> 組
              </p>
            </div>
            <p class="ticketCard-price">
              TWD <span id="ticketCard-price">$${value.ticketCost}</span>
            </p>
          </div>
        </div>
      </li>
...
search.addEventListener('change',() => {
  if (search.value === '台北') {
    const left = ticketData.filter(value => {
      return value.viewLocation === '台北';
    });
    show(left);

  } else if (search.value === '台中') {
    const left = ticketData.filter(value => {
      return value.viewLocation === '台中';
    });
    show(left);

  } else if (search.value === '高雄') {
    const left = ticketData.filter(value => {
      return value.viewLocation === '高雄';
    });
    show(left);
